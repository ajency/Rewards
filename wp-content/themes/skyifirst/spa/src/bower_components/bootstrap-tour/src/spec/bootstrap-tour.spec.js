(function() {
  describe("Bootstrap Tour", function() {
    ...
    it("should ignore `reflex` attribute on orphan steps", function() {
      this.tour = new Tour;
      this.tour.addStep({
        orphan: true,
        reflex: true
      ...
      this.tour.showStep(0);
      return expect($('body').css("cursor")).not.toBe("pointer");
    });
    return it("should not display inactive popover upon rapid navigation", function() {
      var step_displayed;
      ...
      this.tour.addStep({
        element: $("<div></div>").appendTo("body"),
        onShown: function(t) {
          return step_displayed = true;
        }
      });
      runs(function() {
        this.tour.goTo(0);
        return this.tour.goTo(1);
      });
      waitsFor((function() {
        return step_displayed;
